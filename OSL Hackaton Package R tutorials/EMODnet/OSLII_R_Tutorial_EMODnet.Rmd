...
pltbathy <- rasterVis::levelplot(bathy_img,
...
bathymetry.col <- function(n, alpha = 1) {
  if ((n <- as.integer(n[1L])) > 0) {
    ...
    i <- n - j - k
    c(
      if (i > 0) hsv(h = seq.int(from = 40 / 60, to = 40 / 60, length.out = i), alpha = alpha),
...
```
![](fig/EMODnet_R_tutorial_map.PNG)
...
# output maps
png(file = paste('fig/', Sessionid, "_bathymetry2d_isobath.png", sep = ""), width, height)
print(map_isobath)
dev.off()

png(file = paste('fig/', Sessionid, "_bathymetry2d_isobath_alt.png", sep = ""), width, height)
print(pltbathy)
dev.off()

# output 3d visualisations

png(file = paste('fig/', Sessionid, "_bathymetry3d_90.png", sep = ""), width, height)
...
png(file = paste('fig/', Sessionid, "_bathymetry3d_0.png", sep = ""), width, height)
...
png(file = paste('fig/', Sessionid, "_bathymetry3d_180.png", sep = ""), width, height)
...
png(file = paste('fig/', Sessionid, "_bathymetry3d_270.png", sep = ""), width, height)
...
```{r, results = 'hide'}
writeRaster(bathy_img, paste0('data/',"EMODnet_bathymetry_555526224.nc"), format = "CDF", force_v4 = TRUE, compression = 7, overwrite = TRUE)
```
...
geojson_write(layer, file = "data/mpa.geojson")
```
...
getDTMgeojson <- function(name = "DTM_Surveys", xmin = -1, xmax = 1, ymin = 49, ymax = 50) {
  ...
  bbox <- paste(xmin, ymin, xmax, ymax, sep = ",")
  con <- paste0(
    "https://ows.emodnet-bathymetry.eu/wfs?SERVICE=WFS&VERSION=1.0.0&request=GetFeature&typeName=",
...
```{r, results='hide'}
geojson_write(DTM_Surveys, file = "data/bathy.geojson")
```
...
gethumactpoint <- function(name = "shellfish", xmin = -1000, xmax = 1000, ymin = -1000, ymax = 1000) {
  ...
  utils::download.file(con, "data/dat.csv", quiet = TRUE, mode = "wb")
  pipo <- utils::read.csv("data/dat.csv")
...
gethumactpointall <- function(xmin = -1.3, xmax = 0.3, ymin = 49.2, ymax = 49.9) {
  ...
  if (nrow(rez0) > 0) {
    ha1 <- data.frame(
      type = "finfish", status = rez0$status,
  ...
  if (nrow(rez0) > 0) {
    ha1 <- data.frame(
      type = "shellfish", status = rez0$site_typology,
  ...
  if (nrow(rez0) > 0) {
    ha1 <- data.frame(
      type = "dredging", status = rez0$purpose,
  ...
  rez0 <- gethumactpoint(
    "hydrocarbons", ymin, xmin, ymax,
    xmax
  )
  if (nrow(rez0) > 0) {
    ha1 <- data.frame(
      type = "hydrocarbons", status = rez0$status,
  ...
  if (nrow(rez0) > 0) {
    ha1 <- data.frame(
      type = "aggregates", status = rez0$notes,
  ...
  if (nrow(rez0) > 0) {
    ha1 <- data.frame(
      type = "platforms", status = rez0$class,
  ...
  if (nrow(rez0) > 0) {
    ha1 <- data.frame(
      type = "windfarms", status = rez0$status,
...
```{r}
write.csv(ha, file = "data/Humanactivities.csv", row.names = FALSE)
```
...
```{r, results='hide'}
geojson_write(natura2000, file = "data/natura2000.geojson")
```
...
vesseltypes <- cbind.data.frame(
  type_id = c("All", "00", "01", "02", "03", "04", "05", "06", "07", "08", "09", "10", "11", "12"),
  type_name = c(
    "ALL", "OTHER", "FISHING", "SERVICE", "DREDGING", "SAILING", "PLEASURE",
...
for (i in 1:nrow(vesseltypes)) {
  mpa_vessel_density_per_type[vesseltypes[i, "type_name"]] <- createmonthlystack(
    vessel_type = vesseltypes[i, "type_id"],
    xmin = xmin, xmax = xmax,
    ymin = ymin, ymax = ymax
  )
}
...
mpaprocessplot <- function(imgs, mpa, name, unite, logscale) {
  if (dim(imgs)[3] < 12) {
    ...
    pltmean <- rasterVis::levelplot(imgsmean,
      margin = F,
      zscaleLog = logscale, contour = F,
      col.regions = col.l, colorkey = ckey,
      main = list(label = titre, cex = 1)
    ...
    pltbw <- rasterVis::bwplot(crop(imgs, extent(mpa)),
      logscale = T,
...
for (type in types_to_plot) {
  plt_vessel <- mpaprocessplot(
    imgs = mpa_vessel_density_per_type[[type]],
    mpa = mpa,
    name = paste("Vessel density", type, sep = " - "), unite = "hours/month",
    logscale = !(type %in% c("HIGHSPEED", "UNKNOWN"))
  ) # too little data in these types to have a logscale
  ...
  # save images as PNG
  png(file = paste('fig/', Sessionid, "_", wdpaid, "_VESSEL_monthly_composite-", type, ".png", sep = ""), width = 960, height = 960)
  print(img1)
  dev.off()

  png(file = paste('fig/', Sessionid, "_", wdpaid, "_VESSEL_average-", type, ".png", sep = ""), width = 960, height = 960)
  print(img2)
  dev.off()
}
```
...
getEMODnetPhysicsplatformall <- function(xmin = -1.3, xmax = 0.3, ymin = 49.2, ymax = 49.9) {
  ...
  if (nrow(rez0) > 0) {
    physics1 <- data.frame(
      type = "EP_PLATFORMS_SLEV", status = rez0$ProjectsDescr,
  ...
  if (nrow(rez0) > 0) {
    physics1 <- data.frame(
      type = "EP_PLATFORMS_WIND", status = rez0$ProjectsDescr,
      purpose = rez0$ParametersGroupDescr, info = rez0$DataOwner, name = rez0$PlatformID, PltLink = rez0$PlatformInfoLink,
      country = rez0$Country, x = rez0$x, y = rez0$y
    )
    physics <- rbind(physics, physics1)
  ...
  if (nrow(rez0) > 0) {
    physics1 <- data.frame(
      type = "EP_PLATFORMS_WAVE", status = rez0$ProjectsDescr,
      purpose = rez0$ParametersGroupDescr, info = rez0$DataOwner, name = rez0$PlatformID, PltLink = rez0$PlatformInfoLink,
      country = rez0$Country, x = rez0$x, y = rez0$y
    )
    physics <- rbind(physics, physics1)
  ...
  if (nrow(rez0) > 0) {
    physics1 <- data.frame(
      type = "EP_PLATFORMS_TEMP", status = rez0$ProjectsDescr,
      purpose = rez0$ParametersGroupDescr, info = rez0$DataOwner, name = rez0$PlatformID, PltLink = rez0$PlatformInfoLink,
      country = rez0$Country, x = rez0$x, y = rez0$y
    )
    physics <- rbind(physics, physics1)
  ...
  if (nrow(rez0) > 0) {
    physics1 <- data.frame(
      type = "EP_PLATFORMS_HCXX", status = rez0$ProjectsDescr,
      purpose = rez0$ParametersGroupDescr, info = rez0$DataOwner, name = rez0$PlatformID, PltLink = rez0$PlatformInfoLink,
      country = rez0$Country, x = rez0$x, y = rez0$y
    )
    physics <- rbind(physics, physics1)
  ...
  if (nrow(rez0) > 0) {
    physics1 <- data.frame(
      type = "EP_PLATFORMS_MO", status = rez0$ProjectsDescr,
      purpose = rez0$ParametersGroupDescr, info = rez0$DataOwner, name = rez0$PlatformID, PltLink = rez0$PlatformInfoLink,
      country = rez0$Country, x = rez0$x, y = rez0$y
    )
    physics <- rbind(physics, physics1)
  ...
  if (nrow(rez0) > 0) {
    physics1 <- data.frame(
      type = "EP_PLATFORMS_HFR", status = rez0$ProjectsDescr,
      purpose = rez0$ParametersGroupDescr, info = rez0$DataOwner, name = rez0$PlatformID, PltLink = rez0$PlatformInfoLink,
      country = rez0$Country, x = rez0$x, y = rez0$y
    )
    physics <- rbind(physics, physics1)
  ...
  if (nrow(rez0) > 0) {
    physics1 <- data.frame(
      type = "EP_PLATFORMS_FB", status = rez0$ProjectsDescr,
      purpose = rez0$ParametersGroupDescr, info = rez0$DataOwner, name = rez0$PlatformID, PltLink = rez0$PlatformInfoLink,
      country = rez0$Country, x = rez0$x, y = rez0$y
    )
    physics <- rbind(physics, physics1)
  ...
  if (nrow(rez0) > 0) {
    physics1 <- data.frame(
      type = "EP_PLATFORMS_GL", status = rez0$ProjectsDescr,
      purpose = rez0$ParametersGroupDescr, info = rez0$DataOwner, name = rez0$PlatformID, PltLink = rez0$PlatformInfoLink,
      country = rez0$Country, x = rez0$x, y = rez0$y
    )
    physics <- rbind(physics, physics1)
...
write.csv(physics, file = "data/EMODnetPhysics.csv")
```
...
# download the image
con <- paste0(
  "https://ows.emodnet-seabedhabitats.eu/emodnet_view/ows?layers=eusm2019_group&bbox=", bbox,
  "&service=WMS&format=image/png&interpolation=nearest&srs=EPSG:4326&request=GetMap&height=", height, "&width=", width, ""
)
download(con, "fig/img.png", quiet = TRUE, mode = "wb")
img <- brick("fig/img.png")
...
# download the image
con <- paste0(
  "https://ows.emodnet-seabedhabitats.eu/emodnet_view/ows?layers=eusm2019_bio_group&bbox=", bbox,
  "&service=WMS&format=image/png&interpolation=nearest&srs=EPSG:4326&request=GetMap&height=", height, "&width=", width, ""
)
download(con, "fig/img.png", quiet = TRUE, mode = "wb")
img <- brick("fig/img.png")
...
# download the image
con <- paste0(
  "https://drive.emodnet-geology.eu/geoserver/gtk/wms?layers=seabed_substrate_1m&bbox=", bbox,
  "&service=WMS&format=image/png&interpolation=nearest&srs=EPSG:4326&request=GetMap&height=", height, "&width=", width, ""
)
download(con, "fig/img.png", quiet = TRUE, mode = "wb")
img <- brick("fig/img.png")
...
getEMODnetCoastalMigration <- function(name = "tno:coastal_migration_satellite_0_40k", xmin = -1000, xmax = 1000, ymin = -1000, ymax = 1000) {
  ...
  bbox <- paste(xmin, xmax, ymin, ymax, sep = ",")
  con <- paste0(
    "https://drive.emodnet-geology.eu/geoserver/tno/wfs?SERVICE=WFS&VERSION=1.1.0&request=GetFeature&typeName=",
...
getEMODnetCoastalMigrationformall <- function(xmin = -1.3, xmax = 0.3, ymin = 49.2, ymax = 49.9) {
  ...
  if (nrow(rez0) > 0) {
    geology1 <- data.frame(
      type = "tno:coastal_migration_satellite_0_40k", Migration = rez0$changerate, name = rez0$id,
      x = rez0$x, y = rez0$y
    )
...
write.csv(geology, file = "data/EMODnetGeology-CoastalMigration.csv")
```
...
<center><img src="fig/Toolbox_NorthSea.PNG" alt="Select the Geographic Area" width="500"/></center>  
...
fname <- "data/Larus_fuscus_NZ.csv"
...
csv_url <- paste0(
  base_url, service, typeName, resultType,
  viewParams,
...
map_bbox_sel
```
![](fig/Larus_fuscus_bbox_sel_map.PNG)
